...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    const { error } = await supabase.from('leads').update({ is_deleted: true, deleted_at: new Date().toISOString() }).eq('id', req.params.id);
    if (error) throw error;
    res.json({ success: true });
  } catch (err) {
...
router.get('/meta/sources', requireAuth, async (req, res) => {
  try {
    const { data, error } = await supabase.from('acquisition_channels').select('id, channel_name').order('channel_name');
    if (error) throw error;
    res.json(data || []);
  } catch (err) {
...
router.post('/:id/convert', requireAuth, async (req, res) => {
  try {
    ...
    const { error: custErr } = await supabase.from('customers').insert([{ company_id, name, country, email }]);
    if (custErr) throw custErr;
    const { error: leadErr } = await supabase.from('leads').update({ stage: 'Won' }).eq('id', req.params.id);
...
router.post('/:id/follow-ups', requireAuth, async (req, res) => {
  try {
    ...
    const { error: followErr } = await supabase.from('follow_ups').insert([{
      lead_id: req.params.id, company_name, contact_name, follow_up_date, note, assigned_to, is_notified: false
    }]);
    if (followErr) throw followErr;
    const { error: leadErr } = await supabase.from('leads').update({ assigned_to }).eq('id', req.params.id);
    if (leadErr) throw leadErr;
    res.json({ success: true });
  } catch (err) {
...
router.get('/:id/activities', requireAuth, async (req, res) => {
  try {
    const { data, error } = await supabase.from('activities').select('*, profiles(full_name)').eq('lead_id', req.params.id).order('created_at', { ascending: false });
    if (error) throw error;
    res.json(data || []);
  } catch (err) {
...
router.get('/:id/quotations', requireAuth, async (req, res) => {
  try {
    const { data, error } = await supabase.from('quotations').select('id, quotation_number, status, created_at, amount, currency').eq('lead_id', req.params.id).order('created_at', { ascending: false });
    if (error) throw error;
    res.json(data || []);
  } catch (err) {
...
router.post('/:id/activities', requireAuth, async (req, res) => {
  try {
    ...
    data.created_by = req.user.sub;
    const { data: act, error } = await supabase.from('lead_activities').insert([data]).select().single();
    if (error) throw error;
